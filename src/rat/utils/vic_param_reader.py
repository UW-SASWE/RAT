...
class VICParameterFile:
    ...
    def _load_from_config(self):
        ...
        if (self.save_init_state):
            create_directory(os.path.join(config['GLOBAL']['data_dir'],config['BASIN']['region_name'],'basins',self.basin_name,'vic', self.init_state_dir),False)
            self.params['state_file_params']['STATENAME'] = os.path.join(config['GLOBAL']['data_dir'],config['BASIN']['region_name'],'basins',self.basin_name,'vic', self.init_state_dir,'state_')
