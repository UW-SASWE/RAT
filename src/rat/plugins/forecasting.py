    ...
    for iter_vol,vol in enumerate(st_volumes):
        ...
        for iter,inflow in enumerate(forecast_outflow['inflow'].values):
            if(cum_netInflow < vol):
                ...
                # O = 0 if I_E - dels_remaining < 0 else I_E - dels_remaining
                # dels_stored = I_E - O
                # dels_remaining = max(dels_remaining - dels_stored, 0)
                outflow.append(O)
...
def forecast_outflow_for_res(
    ...
    forecast_inflow = pd.read_csv(forecast_inflow_fp, parse_dates=['date']).set_index('date').rename({'streamflow': 'inflow'}, axis=1)
    forecast_inflow['inflow'] = forecast_inflow['inflow'] * 86400
    ...
    forecast_outflow = forecast_inflow.to_xarray().sel(date = slice(base_date, base_date_15lead))
