...
class MSParameterFile:
    def __init__(self, config, init_param, forcings=None, state=None, runname=None):
        self.config = config
    ...
    def _load_from_config(self):
        ...
        self.results = os.path.join(self.params['MetSim']['out_dir'], f"{self.params['MetSim']['out_prefix']}_{start.strftime('%Y%m%d')}-{end.strftime('%Y%m%d')}.nc")
